...
public class GsonUtil {
     ...
     * @return 目标对象的 {@code JSON} 格式的字符串。  java.lang.reflect
     */
...
//            Log.d("目标对象 " + target.getClass().getName()
...
//            Log.d(json + " 无法转换为 " + token.getRawType().getName() + " 对象!");
            return null;
        }
    }
...
//            Log.d(json + " 无法转换为 " + clazz.getName() + " 对象!");
            return null;
        }
    }
    ...
    public static String toJsons(Object target) {
        return new Gson().toJson(target);
    }

    /*common 的开始 */
    public static Common fromCommJson(String json) {
        Common commonBean = new Common();
        try {
            ...
            commonBean.setError(error);
            commonBean.setResults(results);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return commonBean;
